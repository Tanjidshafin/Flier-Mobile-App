...
import { useHomeQuery } from '../features/hotels/hooks';
...
export function HomeScreen({ navigation }: Props) {
  ...
  const { data: payload, error, isLoading, refetch } = useHomeQuery();
    ...
    <View className="flex-1 bg-brand-surfaceMuted">
      <AnimatedScrollView
            ...
            <Pressable
              ...
              <View className="ml-2 flex-1">
                <Text style={[typography.heading, { fontSize: 26 }]} className="text-brand-text">
                  {hasDestination ? destinationLabel : 'Browse modern stays'}
                </Text>
                <Text style={[typography.body, { color: colors.textSecondary, marginTop: 4 }]}>
                  {hasDestination
                    ? 'Search hotels, refine dates, and keep moving.'
                    : 'Choose a city or jump into all hotels to explore.'}
                </Text>
              </View>
            </Pressable>
            <Pressable
              className="h-11 w-11 items-center justify-center rounded-full bg-brand-surfaceMuted"
        ...
        <View className="px-5">
          <SectionHeader
            actionLabel="See all"
            onPressAction={() => openSearch('browse')}
          ...
          {isLoading ? (
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              {payload.featuredHotels.map((hotel, index) => (
                <View className={`${index === 0 ? '' : 'ml-4'} w-[300px]`} key={hotel.id}>
                  <HotelCard
                    compact
                    hotel={hotel}
                    onPress={() =>
                    ...
                    onToggleSaved={() => {
                      void toggleSaved(hotel);
                      showToast({
                        message: isSaved(hotel.id)
                          ? 'Removed from your saved stays.'
                          : 'Added to your saved stays.',
                        title: 'Wishlist',
                        tone: 'success',
                      });
                    }}
            ...
            <EmptyState
              message={
                error instanceof Error
                  ? error.message
                  : 'Seed the backend and featured hotels will show up here.'
              }
            ...
            {payload?.featuredDestinations.length ? (
              <View className="gap-4">
                {payload.featuredDestinations.map(destination => (
                  <DestinationCard
                    ...
                    onPress={() => {
                      setDestination(destination.destinationId, destination.locationLabel, '');
                      openSearch('browse');
                    }}
                  />
                ))}
              ...
              <View className="rounded-[28px] bg-white p-4">
                ...
                <Text style={typography.title} className="mt-4 text-brand-text">
                  Featured destinations are unavailable right now
                </Text>
                <Pressable
                  className="mt-4 rounded-full bg-brand-primary px-4 py-3"
                  onPress={() => {
                    void refetch();
                  }}>
                  <Text style={[typography.button, { color: '#FFFFFF' }]}>Retry</Text>
                </Pressable>
              </View>
            )}
          </View>
